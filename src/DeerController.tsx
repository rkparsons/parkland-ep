...
const DeerController: FC = () => {
  ...
  const walk = useAnimation(
    "Walk_forward_IP",
    () => distVecRef.current >= 10 * translationSpeed
  );
  // todo: add other idle anims on loop
  const idle = useAnimation(
    "Idle_1",
    () => distVecRef.current < 10 * translationSpeed
  );
  ...
  useBeforeRender(() => {
    ...
    const deerWalkAngle = getAngleBetweenMeshes(
      deerRef.current,
      waypointRef.current
    );
    ...
    if (isWalking) {
      ...
      deerRef.current.translate(
        targetVecNormRef.current,
        walkSpeed,
        Space.WORLD
      );
