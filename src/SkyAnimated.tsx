...
//rendering quality 
const int steps = 4; //16 is fast, 128 or 256 is extreme high
...
const SkyAnimated: FC = () => {
    ...
    useBeforeRender(() => {
        shader.current?.setFloat('time', time.current)

        //Animate Move Sun
        shader.current?.setFloat('suny', Math.sin(time.current / 3))
        shader.current?.setFloat('sunx', Math.sin(time.current / 3))
        time.current += 0.008
    })

    return (
        <sphere name="Dome" segments={50} diameter={50}>
            <shaderMaterial
                name="shader"
                ref={shader}
                shaderPath={{
            ...
            >
                <texture
                    name="iChannel0"
                    url="http://i.imgur.com/kUJBvin.png"
                    noMipmap={true}
                    invertY={false}
                    samplingMode={12}
                />
